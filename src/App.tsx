...
export default function Component() {
  ...
  const [allSimulations, setAllSimulations] = useState<{player1: string, results: SimulationResult[], algorithmNames: string[]} | null>(null)
  ...
  const handleRunAgainstAll = (player1: string, iterations: number) => {
    ...
    // Save to local storage
    const savedSimulations = JSON.parse(localStorage.getItem("simulations") || "[]")
    savedSimulations.push({ player1, player2: "All", iterations, results, algorithmNames })
    localStorage.setItem("simulations", JSON.stringify(savedSimulations.slice(-5))) // Keep last 5 simulations
  }
  ...
  const handleLoadSimulation = (simulation: any) => {
    if (Array.isArray(simulation.result)) {
      setAllSimulations({ 
        ...
        algorithmNames: simulation.algorithmNames || simulation.result.map((_: any, i: number) => `Algorithm ${i + 1}`)
      })
      setCurrentSimulation(null)
    } else {
        ...
        <section id="selector">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose your algorithms and run the simulation</CardDescription>
            </CardHeader>
            <CardContent>
              <SimulationSelector onRunSimulation={handleRunSimulation} onRunAgainstAll={handleRunAgainstAll} />
            </CardContent>
          </Card>
        </section>
        ...
        <div className="container mx-auto px-4">
          ...
          <Card className="mt-4 bg-card">
            ...
            <CardContent className="flex justify-center space-x-4">
              ...
              <Button variant="outline" size="icon" onClick={() => window.open("https://linkedin.com/in/vipulsharma2004", "_blank")} aria-label="Developer's LinkedIn profile">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
              </Button>
              <Button variant="outline" size="icon" onClick={() => window.open("https://github.com/vipul-69-ax", "_blank")} aria-label="Developer's GitHub profile">
